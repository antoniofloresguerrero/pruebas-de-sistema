describe('pruebas de la página TODO', () => {
  ...
  it('añadir tarea', () => {
   ...
   it('Editar tarea', () => {
    ...
    cy.contains('[data-testid="todo-item-label"]', "ir al taller").dblclick({ force: true })
    cy.get('[data-testid="text-input"]').clear().type("ir al super{enter}")
    //cy.get('[data-testid="text-input"]').type("ir al super{enter}")
    //cy.get("input.edit").should("exist").clear().type("ir al super{enter}")    
    //cy.contains('[data-testid="todo-item-label"]',"ir al super").should("be.visible")
